...
def price_tool(packet: CalculatedDataPacket, window: int = 250) -> str:
    ...
    for _, row in display.iterrows():
        ...
        o_val = (
            f"{row.get('open_adj', 0):.2f}" if pd.notna(row.get("open_adj")) else "N/A"
        )
        h_val = (
            f"{row.get('high_adj', 0):.2f}" if pd.notna(row.get("high_adj")) else "N/A"
        )
        l_val = (
            f"{row.get('low_adj', 0):.2f}" if pd.notna(row.get("low_adj")) else "N/A"
        )
        c_val = (
            f"{row.get('close_adj', 0):.2f}"
            if pd.notna(row.get("close_adj"))
            else "N/A"
        )
        v_val = f"{row.get('vol', 0):.0f}" if pd.notna(row.get("vol")) else "N/A"
        p_val = (
            f"{row.get('pct_chg', 0):.2f}" if pd.notna(row.get("pct_chg")) else "N/A"
        )
        lines.append(
            f"| {date} | {o_val} | {h_val} | {l_val} | {c_val} | {v_val} | {p_val} |"
        )
    ...
    anomalies = [
        a
        for a in (packet.anomalies or [])
        if a.get("date", "") >= str(df.iloc[-20].get("trade_date", ""))
    ]
    if anomalies:
        lines.append("\n### 近20日异常事件")
...
def indicator_tool(packet: CalculatedDataPacket) -> str:
    ...
    lines.append("| " + " | ".join(_fmt_value(v) for _, v in ma_vals) + " |")
    lines.append(
    ...
    for key in ["ma_cross_5_20", "ma_cross_10_60", "ma_cross_20_60"]:
        if ma.get(key):
            cross_info.append(
                f"{key.replace('ma_cross_', '').replace('_', '/')}均线：{ma[key]}"
            )
    if cross_info:
    ...
    lines.append("\n### 成交量指标")
    lines.append(f"- 量比（当日成交量/5日均量）：{_fmt_value(vi.get('volume_ratio'))}")
    lines.append(f"- 振幅（今日）：{_fmt_value(vi.get('amplitude'))}%")
...
def snapshot_tool(packet: CalculatedDataPacket) -> str:
    ...
    recent_5 = df.tail(5)
    hi_5 = (
        recent_5["high_adj"].max()
        if "high_adj" in recent_5.columns
        else recent_5.get("high", recent_5.get("close_adj", 0)).max()
    )
    lo_5 = (
        recent_5["low_adj"].min()
        if "low_adj" in recent_5.columns
        else recent_5.get("low", recent_5.get("close_adj", 0)).min()
    )
